...
class MemoryGitTest(unittest.TestCase):
    def test_scope_noop_and_staged_guard(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            (v/'bilgi').mkdir()
            (v/'bilgi/sunum-tercihi.md').write_text('Source reviewed note')
            (v/'bilgi/.reviews.jsonl').write_text('{}\n')
            (v/'bilgi/raw.txt').write_text('unrelated')
            ...
            self.assertIn('bilgi/sunum-tercihi.md',tracked)
            self.assertIn('bilgi/.reviews.jsonl',tracked)
            self.assertNotIn('bilgi/raw.txt',tracked)
            self.assertEqual(commit_memory(v)['status'], 'unchanged')
            git('add', 'personal.md')
